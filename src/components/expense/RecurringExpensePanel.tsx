...
import { loadRecurring, addRecurring, removeRecurring, type RecurringTemplate } from "@/lib/recurring";

type Props = {
  onApply: (templates: RecurringTemplate[]) => void;
};

const EXPENSE_CATEGORIES = [
  "วัตถุดิบ", "ค่าแรง", "ค่าเช่า", "ค่าน้ำค่าไฟ",
  "การตลาด", "บรรจุภัณฑ์", "ซ่อมบำรุง", "อื่นๆ",
];
...
export function RecurringExpensePanel({ onApply }: Props) {
  const [open, setOpen] = useState(false);
  const [templates, setTemplates] = useState<RecurringTemplate[]>([]);
  const [adding, setAdding] = useState(false);
  const [form, setForm] = useState({ category: "ค่าเช่า", amount: "", note: "" });

  useEffect(() => {
    setTemplates(loadRecurring());
  }, [open]);

  function handleAdd() {
    const amt = parseFloat(form.amount);
    if (!form.category || isNaN(amt) || amt <= 0) return;
    addRecurring({ category: form.category, amount: amt, note: form.note });
    setTemplates(loadRecurring());
    setForm({ category: "ค่าเช่า", amount: "", note: "" });
    setAdding(false);
  }
  ...
  return (
    <>
      ...
      >
        ...
        <span className="flex-1 text-left">รายจ่ายประจำ</span>
        {templates.length > 0 && (
          ...
          <div className="bg-surface w-full max-w-lg rounded-t-2xl shadow-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="p-6 border-b border-surface-variant flex justify-between items-center shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-headline-md text-headline-md text-on-surface">รายจ่ายประจำ</h3>
            ...
            <div className="p-6 space-y-3 overflow-y-auto flex-1">
              {templates.length === 0 && !adding && (
                <p className="font-body-md text-on-surface-variant text-center py-6">ยังไม่มีรายจ่ายประจำ</p>
              )}
              {templates.map((t) => (
            ...
            {templates.length > 0 && (
              <div className="p-6 border-t border-surface-variant shrink-0">
                ...
                >
                  ...
                  นำเข้ารายจ่ายประจำเดือนนี้ ({templates.length} รายการ)
                </button>
              </div>
            )}
          </div>
